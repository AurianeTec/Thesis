if __name__ == '__main__':
    ...
    def process_node(args):
        ...
        if o == d:
            ...
            demand = (
                ...
                * (g_igraph.vs[centroids[d]]['num_jobs'])
    ...
    active_pop_dens_job_count_noEQ = pd.DataFrame(baseline)
    ...
    def process_combination(COLOFINTEREST1):
        ...
        for delta in delta_list:
            result = equalization_with_2attributes(nodes_carbike_centroids_RER_complete, active_pop_dens_job_count_noEQ, centroids, COLOFINTEREST1, delta)
            results.update(result)
        return results

    COLOFINTEREST1 = ['median_income']
    pool = mp.Pool(processes=num_processes)
    ...
    OD_matrices_names.append('active_pop_dens_job_count_noEQ')
    OD_matrices = [dataframe for sublist in OD_matrix for dataframe in sublist]
    OD_matrices.append(active_pop_dens_job_count_noEQ)
    ...
    dicts_df.to_csv('./data/clean/dicts_jobs.csv')

    
    #### SAVE RESULTS ####
    # Open the output file in append mode
    output_file = "./data/clean/gaps_benefit_metric_jobs.csv"
